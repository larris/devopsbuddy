...
public class ForgotMyPasswordController {
    ...
    public static final String EMAIL_MESSAGE_PROPERTY_NAME = "forgotmypassword.mail.text";
    ...
    public String forgotPasswordPost(HttpServletRequest request, @RequestParam("email") String email, ModelMap model){
        ...
        if (null == passwordResetToken){
            LOG.warn("Couldnt find password reset token for email {}",email);
        } else {
            ...
            emailService.sendGenericEmaiMessage(mailMessage);
        }
    ...
    public String changeUserPasswordGet(@RequestParam("id") long id,
        ...
        if (StringUtils.isEmpty(token) || id == 0){
            LOG.error("Invalid userid {} or token {}",id,token);
        ...
        if (null == passwordResetToken){
            LOG.warn("A token could not be found with the value {}",token);
            model.addAttribute(PASSWORD_RESET_ATTRIBUTE_NAME,"false");
        ...
        if (user.getId() != id){
            LOG.error("The user id {} passed as a parameter does not match the use id {} associated with the token {} ",id,user.getId(),token);
            model.addAttribute(PASSWORD_RESET_ATTRIBUTE_NAME,"false");
    ...
    public String changeUserPasswordPost(@RequestParam("prinicpal_id") long userId,
                                         @RequestParam("password") String password,
                                         ModelMap model){

        Authentication authentication =SecurityContextHolder.getContext().getAuthentication();
        if (null == authentication){
            LOG.error("An unauthenticated user tried to invoke the POST method");
            model.addAttribute(PASSWORD_RESET_ATTRIBUTE_NAME,"false");
        ...
        if(user.getId() == userId){
            LOG.error("Security breach !! User {} is trying to make a passowrd reset on behalf of {}",user.getId(),userId);
            model.addAttribute(PASSWORD_RESET_ATTRIBUTE_NAME,"false");
            model.addAttribute(MESSAGE_ATTRIBUTE_NAME,"You are not authorized to perform this request");
            return  CHANGE_PASSWORD_VIEW_NAME;
