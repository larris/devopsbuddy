...
public class PasswordResetToken implements Serializable {
     ...
     * @param user The user for witch the token must be created. Not null
     * @param creationDateTime The date time when this request was created. Not null
     * @param expirationInMinutes Length in minutes for witch the token will be valid. if zero the default value is 120 minutes
     */
    public PasswordResetToken(String token, User user, LocalDateTime creationDateTime,int expirationInMinutes) {
        ...
        if (expirationInMinutes == 0){
           LOG.warn("The token expiration length in minutes is 0 . Assigning the default value {}  ", DEFAULT_TOKEN_LENGTH_IN_MINUTES);
